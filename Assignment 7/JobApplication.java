public class JobApplication implements Comparable<JobApplication> {
	...
	public int compareTo(JobApplication other) {
		if (priority < other.getPriority()) {
			...
			if (availableStartDate.compareTo(other.availableStartDate) > 0) {
				return 1;
			} else if (availableStartDate.compareTo(other.availableStartDate) == 0) {
				return 0;
			} else {
				return -1;
			}
	...
	public boolean equals(JobApplication other) {
		if (name == other.getName() && priority == other.getPriority()) {
			return true;
		} else {
			return false;
		}
